...
const generateSlopeDataReport = async (
  ...
  markdown += `- **SMA(15)**: ${data.sma15 !== null ? await self.exchangeService.formatPrice(symbol, data.sma15) + ' USD' : 'N/A'} (over 15 candles, 15min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  markdown += `- **EMA(15)**: ${data.ema15 !== null ? await self.exchangeService.formatPrice(symbol, data.ema15) + ' USD' : 'N/A'} (over 15 candles, 15min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  markdown += `- **Momentum(10)**: ${data.momentum10 !== null ? await self.exchangeService.formatPrice(symbol, data.momentum10) + ' USD' : 'N/A'} (over 10 candles, 10min on 1m timeframe, Min: -∞ USD, Max: +∞ USD)\n`;
  ...
  markdown += `- **VWAP**: ${await self.exchangeService.formatPrice(symbol, data.vwap)} USD (over 120 candles, 2h on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  markdown += `- **VMA(15)**: ${await self.exchangeService.formatQuantity(symbol, data.vma15)} (over 15 candles, 15min on 1m timeframe, Min: 0, Max: +∞)\n`;
  ...
  public getSlopeData = ttl(
    async (symbol: string): Promise<ISlopeData> => {
      ...
      const slope = candles.reduce(
        (acc, candle) => {
          const { close, volume, timestamp } = candle;
          acc.prices.push([timestamp, close]);
          acc.total_volumes.push([timestamp, volume]);
          return acc;
        },
        {
