...
async function generateComprehensiveReport(symbol: string, analysis: IShortTermAnalysis, self: ShortTermMathService): Promise<string> {
  ...
  report += `- **Support Level**: ${await self.exchangeService.formatPrice(symbol, analysis.support)} USD (over 48 candles, 12h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Resistance Level**: ${await self.exchangeService.formatPrice(symbol, analysis.resistance)} USD (over 48 candles, 12h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Nearest Fibonacci Level**: ${analysis.fibonacci.nearestLevel.level} at ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.nearestLevel.price)} USD (${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.nearestLevel.distance)} USD away)\n\n`;
  ...
  report += `- **Bollinger Upper(10,2.0)**: ${analysis.bollingerUpper10_2 !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollingerUpper10_2) : 'N/A'} USD (over 10 candles, 2.5h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Middle(10,2.0)**: ${analysis.bollingerMiddle10_2 !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollingerMiddle10_2) : 'N/A'} USD (over 10 candles, 2.5h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Lower(10,2.0)**: ${analysis.bollingerLower10_2 !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollingerLower10_2) : 'N/A'} USD (over 10 candles, 2.5h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **ATR(9)**: ${analysis.atr9 !== null ? await self.exchangeService.formatPrice(symbol, analysis.atr9) : 'N/A'} USD (over 9 candles, 2.25h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **SMA(50)**: ${analysis.sma50 !== null ? await self.exchangeService.formatPrice(symbol, analysis.sma50) : 'N/A'} USD (over 50 candles, 12.5h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(8)**: ${analysis.ema8 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema8) : 'N/A'} USD (over 8 candles, 2h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(21)**: ${analysis.ema21 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema21) : 'N/A'} USD (over 21 candles, 5.25h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **DEMA(21)**: ${analysis.dema21 !== null ? await self.exchangeService.formatPrice(symbol, analysis.dema21) : 'N/A'} USD (over 21 candles, 5.25h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **WMA(20)**: ${analysis.wma20 !== null ? await self.exchangeService.formatPrice(symbol, analysis.wma20) : 'N/A'} USD (over 20 candles, 5h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Momentum(8)**: ${analysis.momentum8 !== null ? await self.exchangeService.formatPrice(symbol, analysis.momentum8) : 'N/A'} USD (over 8 candles, 2h on 15m timeframe, Min: -∞ USD, Max: +∞ USD)\n`;
  ...
  report += `- **0.0% (High)**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["0.0%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **23.6%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["23.6%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **38.2%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["38.2%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **50.0%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["50.0%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **61.8%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["61.8%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **78.6%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["78.6%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **100.0% (Low)**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["100.0%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **127.2% Extension**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["127.2%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **161.8% Extension**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["161.8%"])} USD (over 288 candles, 72h on 15m timeframe, Min: 0 USD, Max: +∞)\n`;

  return report;
}

// Simplified analysis using only trading-signals library and essential custom logic
function generateAnalysis(candles: ICandleData[]): IShortTermAnalysis {
  const closes = candles.map((candle) => Number(candle.close));
  const highs = candles.map((candle) => Number(candle.high));
  const lows = candles.map((candle) => Number(candle.low));
...
export class ShortTermMathService {
  ...
  public getShortTermAnalysis = ttl(
    async (symbol: string): Promise<IShortTermAnalysis> => {
      ...
      const candles: ICandleData[] = await this.exchangeService.getCandles(symbol, "15m", 144);
