...
function calculateFibonacciLevels(candles: ICandleData[], period: number = 48): IFibonacciLevels {
...
async function generateComprehensiveReport(analysis: ISwingTermAnalysis, self: SwingTermMathService): Promise<string> {
  ...
  report += `- **Volume**: ${analysis.volume ? await self.exchangeService.formatQuantity(analysis.symbol, analysis.volume) : "N/A"} ${volumeUnit} (current candle, Min: 0, Max: +∞)\n`;
  report += `- **Support Level**: ${analysis.support ? await self.exchangeService.formatPrice(analysis.symbol, analysis.support) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Resistance Level**: ${analysis.resistance ? await self.exchangeService.formatPrice(analysis.symbol, analysis.resistance) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Price Momentum**: ${analysis.priceMomentum6 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.priceMomentum6) + ' USD' : "N/A"} (Min: -∞ USD, Max: +∞ USD, over 6 candles, 3h on 30m timeframe)\n\n`;
  ...
  report += `- **ATR(14)**: ${analysis.atr14 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.atr14) : "N/A"} USD (over 14 candles, 7h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Band Width**: ${analysis.bollingerBandWidth20_2?.toFixed(2) ?? "N/A"}% (over 20 candles, 10h on 30m timeframe, Min: 0%, Max: +∞)\n`;
  report += `- **Bollinger Upper(20,2.0)**: ${analysis.bollinger20_2.upper ? await self.exchangeService.formatPrice(analysis.symbol, analysis.bollinger20_2.upper) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Middle(20,2.0)**: ${analysis.bollinger20_2.middle ? await self.exchangeService.formatPrice(analysis.symbol, analysis.bollinger20_2.middle) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Lower(20,2.0)**: ${analysis.bollinger20_2.lower ? await self.exchangeService.formatPrice(analysis.symbol, analysis.bollinger20_2.lower) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **SMA(20)**: ${analysis.sma20 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.sma20) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(13)**: ${analysis.ema13 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.ema13) : "N/A"} USD (over 13 candles, 6.5h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(34)**: ${analysis.ema34 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.ema34) : "N/A"} USD (over 34 candles, 17h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **DEMA(21)**: ${analysis.dema21 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.dema21) : "N/A"} USD (over 21 candles, 10.5h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **WMA(20)**: ${analysis.wma20 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.wma20) : "N/A"} USD (over 20 candles, 10h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Momentum(8)**: ${analysis.momentum8 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.momentum8) + ' USD' : "N/A"} (over 8 candles, 4h on 30m timeframe, Min: -∞ USD, Max: +∞ USD)\n`;
  ...
  report += `- **0.0% (High)**: ${analysis.fibonacci.retracement.level0 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level0) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **23.6%**: ${analysis.fibonacci.retracement.level236 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level236) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **38.2%**: ${analysis.fibonacci.retracement.level382 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level382) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **50.0%**: ${analysis.fibonacci.retracement.level500 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level500) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **61.8%**: ${analysis.fibonacci.retracement.level618 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level618) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **78.6%**: ${analysis.fibonacci.retracement.level786 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level786) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **100% (Low)**: ${analysis.fibonacci.retracement.level1000 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.retracement.level1000) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **127.2% Extension**: ${analysis.fibonacci.extension.level1272 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.extension.level1272) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **161.8% Extension**: ${analysis.fibonacci.extension.level1618 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.extension.level1618) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **261.8% Extension**: ${analysis.fibonacci.extension.level2618 ? await self.exchangeService.formatPrice(analysis.symbol, analysis.fibonacci.extension.level2618) : "N/A"} USD (over 48 candles, 24h on 30m timeframe, Min: 0 USD, Max: +∞)\n`;
...
function generateAnalysis(
  ...
  const recentCandles = candles.slice(-RECENT_CANDLES).map((candle) => {
    ...
    try {
      const closeTime = Number(candle.timestamp);
...
export class SwingTermMathService {
  public readonly exchangeService = inject<ExchangeService>(
  ...
  public getSwingTermAnalysis = ttl(
    async (symbol: string): Promise<ISwingTermAnalysis> => {
      ...
      const candles: ICandleData[] = await this.exchangeService.getCandles(
        symbol,
        "30m",
        96
      );
