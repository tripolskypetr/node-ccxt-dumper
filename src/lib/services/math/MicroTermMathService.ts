...
function calculateVolumeMetrics(candles: ICandleData[]): {
...
function calculatePriceChanges(candles: ICandleData[]): {
...
function calculateSupportResistance(candles: ICandleData[], currentPrice: number): {
  support: number | null;
...
function analyzeCandles(candles: ICandleData[]): IMicroTermAnalysis['recentCandles'] {
...
function generateMicroTermAnalysis(candles: ICandleData[]): IMicroTermAnalysis {
  const closes = candles.map(c => c.close);
  const highs = candles.map(c => c.high);
  const lows = candles.map(c => c.low);
  const volumes = candles.map(c => c.volume);
  ...
  const dataQuality = {
    ...
    dataAge: candles.length > 0 ? Math.floor((Date.now() - candles[candles.length - 1].timestamp) / 1000) : 0
  };
...
async function generateMicroTermReport(
  ...
  report += `- **Current Price**: ${analysis.currentPrice !== null ? await self.exchangeService.formatPrice(symbol, analysis.currentPrice) : 'N/A'} USD (current candle, Min: 0 USD, Max: +∞)\n`;
  ...
  report += `- **Momentum(5)**: ${analysis.momentum5 !== null ? await self.exchangeService.formatPrice(symbol, analysis.momentum5) : 'N/A'} USD (over 5 candles, 5min on 1m timeframe, Min: -∞ USD, Max: +∞ USD)\n`;
  report += `- **Momentum(10)**: ${analysis.momentum10 !== null ? await self.exchangeService.formatPrice(symbol, analysis.momentum10) : 'N/A'} USD (over 10 candles, 10min on 1m timeframe, Min: -∞ USD, Max: +∞ USD)\n`;
  ...
  report += `- **Bollinger Upper(8,2.0)**: ${analysis.bollingerUpper8_2 !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollingerUpper8_2) : 'N/A'} USD (over 8 candles, 8min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Middle(8,2.0)**: ${analysis.bollingerMiddle8_2 !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollingerMiddle8_2) : 'N/A'} USD (over 8 candles, 8min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Lower(8,2.0)**: ${analysis.bollingerLower8_2 !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollingerLower8_2) : 'N/A'} USD (over 8 candles, 8min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  ...
  report += `- **ATR(5)**: ${analysis.atr5 !== null ? await self.exchangeService.formatPrice(symbol, analysis.atr5) : 'N/A'} USD (over 5 candles, 5min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **ATR(9)**: ${analysis.atr9 !== null ? await self.exchangeService.formatPrice(symbol, analysis.atr9) : 'N/A'} USD (over 9 candles, 9min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Volatility(5)**: ${analysis.volatility5?.toFixed(3) ?? 'N/A'}% (over 5 candles, 5min on 1m timeframe, Min: 0%, Max: +∞)\n`;
  report += `- **True Range**: ${analysis.trueRange !== null ? await self.exchangeService.formatPrice(symbol, analysis.trueRange) : 'N/A'} USD (current candle, Min: 0 USD, Max: +∞)\n\n`;
  ...
  report += `- **EMA(3)**: ${analysis.ema3 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema3) : 'N/A'} USD (over 3 candles, 3min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(8)**: ${analysis.ema8 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema8) : 'N/A'} USD (over 8 candles, 8min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(13)**: ${analysis.ema13 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema13) : 'N/A'} USD (over 13 candles, 13min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(21)**: ${analysis.ema21 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema21) : 'N/A'} USD (over 21 candles, 21min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **SMA(8)**: ${analysis.sma8 !== null ? await self.exchangeService.formatPrice(symbol, analysis.sma8) : 'N/A'} USD (over 8 candles, 8min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **DEMA(8)**: ${analysis.dema8 !== null ? await self.exchangeService.formatPrice(symbol, analysis.dema8) : 'N/A'} USD (over 8 candles, 8min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **WMA(5)**: ${analysis.wma5 !== null ? await self.exchangeService.formatPrice(symbol, analysis.wma5) : 'N/A'} USD (over 5 candles, 5min on 1m timeframe, Min: 0 USD, Max: +∞)\n\n`;

  // Support/Resistance levels
  report += `## Support/Resistance Levels\n`;
  report += `- **Support**: ${analysis.support !== null ? await self.exchangeService.formatPrice(symbol, analysis.support) : 'N/A'} USD (over 30 candles, 30min on 1m timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Resistance**: ${analysis.resistance !== null ? await self.exchangeService.formatPrice(symbol, analysis.resistance) : 'N/A'} USD (over 30 candles, 30min on 1m timeframe, Min: 0 USD, Max: +∞)\n\n`;
...
export class MicroTermMathService {
  ...
  public getMicroTermAnalysis = ttl(
    async (symbol: string): Promise<IMicroTermAnalysis> => {
      ...
      const candles: ICandleData[] = await this.exchangeService.getCandles(symbol, "1m", 60);
