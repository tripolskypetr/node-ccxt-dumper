...
function calculateTechnicalIndicators(data: ICandleData[]): TechnicalIndicators[] {
...
function calculatePivotPoints(data: ICandleData[]): PivotPointLevel[] {
...
function analyzeVolume(data: ICandleData[], threshold: number = 1.5): SignificantVolumeLevel[] {
...
const generateEnhancedVolumeReport = async (
  ...
  if (result.technicalIndicators.length > 0) {
    ...
    const price = !isUnsafe(latest.price) ? await self.exchangeService.formatPrice(symbol, latest.price) : 'N/A';
    const sma20 = latest.sma20 !== null ? await self.exchangeService.formatPrice(symbol, latest.sma20) : 'N/A';
    const sma50 = latest.sma50 !== null ? await self.exchangeService.formatPrice(symbol, latest.sma50) : 'N/A';
    const sma200 = latest.sma200 !== null ? await self.exchangeService.formatPrice(symbol, latest.sma200) : 'N/A';
    const ema12 = latest.ema12 !== null ? await self.exchangeService.formatPrice(symbol, latest.ema12) : 'N/A';
    const ema26 = latest.ema26 !== null ? await self.exchangeService.formatPrice(symbol, latest.ema26) : 'N/A';
    const ema50 = latest.ema50 !== null ? await self.exchangeService.formatPrice(symbol, latest.ema50) : 'N/A';
    const dema21 = latest.dema21 !== null ? await self.exchangeService.formatPrice(symbol, latest.dema21) : 'N/A';
    const wma20 = latest.wma20 !== null ? await self.exchangeService.formatPrice(symbol, latest.wma20) : 'N/A';
    const bollinger20_2_upper = latest.bollinger20_2_upper !== null ? await self.exchangeService.formatPrice(symbol, latest.bollinger20_2_upper) : 'N/A';
    const bollinger20_2_lower = latest.bollinger20_2_lower !== null ? await self.exchangeService.formatPrice(symbol, latest.bollinger20_2_lower) : 'N/A';
    ...
    markdown += `| Momentum(10) | ${latest.momentum10 !== null ? await self.exchangeService.formatPrice(symbol, latest.momentum10) : 'N/A'} USD (Min: -∞ USD, Max: +∞ USD) | over 10 candles, 10h on 1h timeframe |\n`;
  ...
  for (const level of result.pivotPoints.slice(-10)) {
    ...
    const pivot = !isUnsafe(level.pivot) ? await self.exchangeService.formatPrice(symbol, level.pivot) : 'N/A';
    const s1 = !isUnsafe(level.support1) ? await self.exchangeService.formatPrice(symbol, level.support1) : 'N/A';
    const r1 = !isUnsafe(level.resistance1) ? await self.exchangeService.formatPrice(symbol, level.resistance1) : 'N/A';
    const s2 = !isUnsafe(level.support2) ? await self.exchangeService.formatPrice(symbol, level.support2) : 'N/A';
    const r2 = !isUnsafe(level.resistance2) ? await self.exchangeService.formatPrice(symbol, level.resistance2) : 'N/A';
    const s3 = !isUnsafe(level.support3) ? await self.exchangeService.formatPrice(symbol, level.support3) : 'N/A';
    const r3 = !isUnsafe(level.resistance3) ? await self.exchangeService.formatPrice(symbol, level.resistance3) : 'N/A';
  ...
  for (const level of result.significantVolumes.slice(-15)) {
    ...
    const price = !isUnsafe(priceValue) ? await self.exchangeService.formatPrice(symbol, priceValue) : 'N/A';
    const volume = !isUnsafe(volumeValue) ? await self.exchangeService.formatQuantity(symbol, volumeValue) : 'N/A';
    const volumeMA = !isUnsafe(level.volumeMA) ? await self.exchangeService.formatQuantity(symbol, level.volumeMA) : 'N/A';
...
export class VolumeDataMathService {
  ...
  public getVolumeDataAnalysis = ttl(async (symbol: string): Promise<EnhancedSMAResult> => {
    ...
    const longTermData = await this.exchangeService.getCandles(symbol, "1h", LONG_TERM_CANDLES);
