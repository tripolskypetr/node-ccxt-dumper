...
function calculateFibonacciLevels(candles: ICandleData[]): IFibonacciLevels {
...
async function generateComprehensiveReport(symbol: string, analysis: ILongTermAnalysis, self: LongTermMathService): Promise<string> {
  ...
  report += `- **Support Level**: ${await self.exchangeService.formatPrice(symbol, analysis.support)} USD (over 4 candles, 4h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Resistance Level**: ${await self.exchangeService.formatPrice(symbol, analysis.resistance)} USD (over 4 candles, 4h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Nearest Fibonacci Level**: ${analysis.fibonacci.nearestLevel.level} at ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.nearestLevel.price)} USD (${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.nearestLevel.distance)} USD away) (24h lookback)\n\n`;
  ...
  report += `- **Bollinger Upper(20,2.0)**: ${analysis.bollinger20_2_upper !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollinger20_2_upper) : 'N/A'} USD (over 20 candles, 20h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Middle(20,2.0)**: ${analysis.bollinger20_2_middle !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollinger20_2_middle) : 'N/A'} USD (over 20 candles, 20h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Bollinger Lower(20,2.0)**: ${analysis.bollinger20_2_lower !== null ? await self.exchangeService.formatPrice(symbol, analysis.bollinger20_2_lower) : 'N/A'} USD (over 20 candles, 20h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **ATR(14)**: ${analysis.atr14 !== null ? await self.exchangeService.formatPrice(symbol, analysis.atr14) : 'N/A'} USD (over 14 candles, 14h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **ATR(14) Raw**: ${analysis.atr14_raw?.toFixed(4) ?? 'N/A'} (over 14 candles, 14h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **ATR(20)**: ${analysis.atr20 !== null ? await self.exchangeService.formatPrice(symbol, analysis.atr20) : 'N/A'} USD (over 20 candles, 20h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **SMA(50)**: ${analysis.sma50 !== null ? await self.exchangeService.formatPrice(symbol, analysis.sma50) : 'N/A'} USD (over 50 candles, 50h on 1h timeframe, calculated from 100h dataset, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(20)**: ${analysis.ema20 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema20) : 'N/A'} USD (over 20 candles, 20h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **EMA(34)**: ${analysis.ema34 !== null ? await self.exchangeService.formatPrice(symbol, analysis.ema34) : 'N/A'} USD (over 34 candles, 34h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **DEMA(21)**: ${analysis.dema21 !== null ? await self.exchangeService.formatPrice(symbol, analysis.dema21) : 'N/A'} USD (over 21 candles, 21h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **WMA(20)**: ${analysis.wma20 !== null ? await self.exchangeService.formatPrice(symbol, analysis.wma20) : 'N/A'} USD (over 20 candles, 20h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **Momentum(10)**: ${analysis.momentum10 !== null ? await self.exchangeService.formatPrice(symbol, analysis.momentum10) : 'N/A'} USD (over 10 candles, 10h on 1h timeframe, Min: -∞ USD, Max: +∞ USD)\n`;
  ...
  report += `- **0.0% (High)**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["0.0%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **23.6%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["23.6%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **38.2%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["38.2%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **50.0%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["50.0%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **61.8%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["61.8%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **78.6%**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["78.6%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **100.0% (Low)**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["100.0%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **127.2% Extension**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["127.2%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;
  report += `- **161.8% Extension**: ${await self.exchangeService.formatPrice(symbol, analysis.fibonacci.levels["161.8%"])} USD (over 24 candles, 24h on 1h timeframe, Min: 0 USD, Max: +∞)\n`;

  return report;
}


function generateAnalysis(candles: ICandleData[], fullCandles?: ICandleData[]): ILongTermAnalysis {
  const closes = candles.map((candle) => Number(candle.close));
  ...
  const recentCandles = candles.slice(-availableCandles).map((candle) => ({
    time: new Date(Number(candle.timestamp)).toISOString(),
...
export class LongTermMathService {
  public readonly exchangeService = inject<ExchangeService>(TYPES.exchangeService);
  ...
  public getLongTermAnalysis = ttl(
    async (symbol: string): Promise<ILongTermAnalysis> => {
      ...
      const fullCandles: ICandleData[] = await this.exchangeService.getCandles(symbol, "1h", 100);
      const candles: ICandleData[] = fullCandles.slice(-48);
