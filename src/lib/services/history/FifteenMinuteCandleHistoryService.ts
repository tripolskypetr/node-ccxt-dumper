...
export class FifteenMinuteCandleHistoryService {
  ...
  public generateFifteenMinuteCandleHistory = async (
    ...
    const candles = await this.exchangeService.getCandles(symbol, "15m", RECENT_CANDLES);
    ...
    for (let index = 0; index < recentCandles.length; index++) {
      ...
      report += `- **Open**: ${await this.exchangeService.formatPrice(symbol, candle.open)} USD\n`;
      report += `- **High**: ${await this.exchangeService.formatPrice(symbol, candle.high)} USD\n`;
      report += `- **Low**: ${await this.exchangeService.formatPrice(symbol, candle.low)} USD\n`;
      report += `- **Close**: ${await this.exchangeService.formatPrice(symbol, candle.close)} USD\n`;
      report += `- **Volume**: ${await this.exchangeService.formatQuantity(symbol, candle.volume)}\n`;
